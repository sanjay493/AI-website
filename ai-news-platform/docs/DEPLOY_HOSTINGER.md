...
The stack can pull **RSS/Atom** sources (defaults include arXiv, Google AI blog, Google News AI topic, plus a curated YouTube channel feed) and optionally **YouTube regional trending** videos when **`YOUTUBE_API_KEY`** is set (YouTube Data API v3, `videos.list` with `chart=mostPopular`; default category **28** = Science & Technology). By default **only trending clips matching AI cues in title, short description, or snippet tags** are kept (substring phrases plus whole-word/tag patterns like `#ai`, GPT, Gemini; optionally extend with `YOUTUBE_TRENDING_AI_KEYWORDS`; disable filtering with **`YOUTUBE_TRENDING_AI_ONLY=false`**). If nothing matches, try **`YOUTUBE_TRENDING_MAX_RESULTS=50`**, **`YOUTUBE_TRENDING_VIDEO_CATEGORY_ID=`** (unset = all trending categories — noisier), or add keywords. Items are stored as **`news`** posts (title, description snippet, link — not full video transcripts).
